...
//
// The engine itself (the af2 search, the XiongMai UART actuator, the worker and
// magnification-reader threads, the /tmp/btzoom.lock discipline, dead-reckoning)
// lives in engine.c + af2.c — ported from majestic's src/af.c/af2.c. This file
// is only the thin adapter from the two-token command ABI to that engine.
